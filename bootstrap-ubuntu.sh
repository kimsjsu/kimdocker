...
RUN tr -d '\015' < /tmp/copied/service-script-mongod > /etc/init.d/mongod && tr -d '\015' < /tmp/copied/mongod.conf > /etc/mongod.conf && tr -d '\015' < /tmp/copied/setup-replication.sh > /tmp/setup-replication.sh && rm -fr /tmp/copied && chmod 755 /etc/init.d/mongod /tmp/setup-replication.sh && update-rc.d mongod defaults
EOF
