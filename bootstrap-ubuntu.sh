...
RUN tr -d '\015' < /tmp/copied/service-script-mongod > /etc/init.d/mongod && tr -d '\015' < /tmp/copied/mongod.conf > /etc/mongod.conf && rm -fr /tmp/copied && chmod 755 /etc/init.d/mongod && update-rc.d mongod defaults
EOF
