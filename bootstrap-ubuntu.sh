...
RUN tr -d '\015' < /tmp/copied/service-script-mongod > /etc/init.d/mongod && tr -d '\015' < /tmp/copied/mongod.conf > /etc/mongod.conf && tr -d '\015' < /tmp/copied/setup_replication.sh > /tmp/setup_replication.sh && rm -fr /tmp/copied && chmod 755 /etc/init.d/mongod /tmp/setup_replication.sh && update-rc.d mongod defaults
EOF
